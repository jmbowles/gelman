...
y1 <- as.matrix (read.table ("dogs2.dat"), nrows=30, ncol=25)
y <- ifelse (y1[,]=="S",1,0)
...
fit.logit.1 <- openbugs(data, inits, parameters, "dogs.logit.1.bug", n.chains=3, n.iter=2000, n.thin=100)

attach.bugs(fit.logit.1)
...
plot (c(0, n.trials-1), c(0,1), xlab="Time", ylab="Proportion of avoidances",type="n")
mtext ("data and replicated data\n(logit link)", 3)

for (s in 1:20){
  #lines (0:(n.trials-1), test (y.sim[s,,]), lwd=.5, col="gray")
  lines (0:(n.trials-1), test(y.rep[s,,]), lwd=.5, col="gray")
}
...
for (s in 1:20){
  lines (0:(n.trials-1), test.diff (y, y.rep[s,,]), lwd=.5, col="gray")
}
abline (0, 0, lwd=3)
